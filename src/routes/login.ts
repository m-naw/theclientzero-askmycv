...
export async function handleLoginGet(
  ...
  return htmlResponse(renderLoginForm({ next, error: info }), { status: 200 });
}
...
export async function handleLoginPost(
  ...
  if (!valid) {
    ...
    return htmlResponse(
      renderLoginForm({ error: "Invalid password. Please try again.", next: safeNext !== "/" ? safeNext : undefined }),
      { status: 401 },
    );
  }
