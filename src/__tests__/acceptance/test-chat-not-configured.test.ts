...
async function clearKv(): Promise<void> {
  ...
  for (const key of [
    ...
    "setup_window_start",
    `spend:${today}`,
    `ratelimit:10.99.0.1:${today}-${hour}`,
  ]) {
    await kv.delete(key);
  }
}
...
describe("Acceptance — POST /chat with empty KV returns 503 not configured (spec §12)", () => {
  beforeEach(async () => {
  ...
  it("POST /chat with empty KV returns HTTP 503 and JSON {error: 'not configured'}", async () => {
