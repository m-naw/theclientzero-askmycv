...
describe("GET /admin — session-primary auth model", () => {
  ...
  // S6: no session cookie → 303 redirect to /login?next= (session is the primary auth layer)
  it("missing session cookie returns 303 redirect to /login?next= regardless of JWT", async () => {
    const kp = await createJwtHarness();
    await getEnv().STATE.put(TEST_JWKS_KV_KEY, JSON.stringify(kp.jwksDocument));
    const cfg = baseConfig();
    await seedConfig(cfg);
    ...
    // No session cookie, JWT present — should redirect to /login?next= (changed in fix-admin-no-session-303-redirect)
